...
import modules.ldm.modules.image_degradation.utils_image as util
...
def add_blur(img, sf=4):
    ...
    if random.random() < 0.5:
        ...
        k = anisotropic_Gaussian(ksize=random.randint(
            2, 11) + 3, theta=random.random() * np.pi, l1=l1, l2=l2)
    else:
        k = fspecial('gaussian', random.randint(
            2, 4) + 3, wd * random.random())
...
def add_Gaussian_noise(img, noise_level1=2, noise_level2=25):
    ...
    rnum = np.random.rand()
    if rnum > 0.6:  # add color Gaussian noise
        img = img + np.random.normal(0, noise_level /
                                     255.0, img.shape).astype(np.float32)
    elif rnum < 0.4:  # add grayscale Gaussian noise
        img = img + np.random.normal(0, noise_level /
                                     255.0, (*img.shape[:2], 1)).astype(np.float32)
    else:  # add  noise
        L = noise_level2 / 255.
        D = np.diag(np.random.rand(3))
        U = orth(np.random.rand(3, 3))
    ...
    for i in shuffle_order:
        ...
        if i == 0:
            ...
            if random.random() < 0.75:
                ...
                img = ndimage.convolve(img, np.expand_dims(
    ...
    for i in shuffle_order:
        ...
        if i == 0:
            ...
            if random.random() < 0.8:
                ...
                k_shifted = shift_pixel(k, sf)
                k_shifted = k_shifted / k_shifted.sum()  # blur with shifted kernel
                image = ndimage.convolve(image, np.expand_dims(
                    k_shifted, axis=2), mode='mirror')
...
if __name__ == '__main__':
    ...
    for i in range(20):
        ...
        lq_bicubic_nearest = cv2.resize(util.single2uint(img_lq_bicubic),
                                        (int(sf * img_lq.shape[1]),
                                         int(sf * img_lq.shape[0])),
                                        interpolation=0)
