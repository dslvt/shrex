...
from modules.cf.models.yolo.yolo import Model
from modules.cf.models.yolo.utils import letterbox, check_img_size, non_max_suppression_face, scale_coords, scale_coords_landmarks

IS_HIGH_VERSION = tuple(
    map(int, torch.__version__.split('+')[0].split('.')[:3])) >= (1, 9, 0)
...
class YoloDetector:
    ...
    def _preprocess(self, imgs):
        ...
        for img in imgs:
            ...
            imgsz = check_img_size(
                max(img.shape[:2]), s=self.detector.stride.max())  # check img_size
    ...
    def _postprocess(self, imgs, origimgs, pred, conf_thres, iou_thres):
        ...
        for image_id, origimg in enumerate(origimgs):
            ...
            scale_coords(imgs[image_id].shape[1:],
                         det[:, :4], img_shape).round()
            scale_coords_landmarks(
                imgs[image_id].shape[1:], det[:, 5:15], img_shape).round()

            for j in range(det.size()[0]):
                box = (det[j, :4].view(1, 4) / gn).view(-1).tolist()
                box = list(
                    map(int, [box[0] * image_width, box[1] * image_height,
                        box[2] * image_width, box[3] * image_height])
                )
                if box[3] - box[1] < self.min_face:
                    continue
                lm = (det[j, 5:15].view(1, 10) / gn_lks).view(-1).tolist()
                lm = list(map(int, [i * image_width if j % 2 ==
                          0 else i * image_height for j, i in enumerate(lm)]))
