...
class BiSeNetOutput(nn.Module):
    ...
    def __init__(self, in_chan, mid_chan, num_class):
        ...
        self.conv_out = nn.Conv2d(
            mid_chan, num_class, kernel_size=1, bias=False)
...
class BiSeNet(nn.Module):
    ...
    def forward(self, x, return_feat=False):
        ...
        out16 = F.interpolate(
            out16, (h, w), mode='bilinear', align_corners=True)
        out32 = F.interpolate(
            out32, (h, w), mode='bilinear', align_corners=True)

        if return_feat:
            feat = F.interpolate(
                feat, (h, w), mode='bilinear', align_corners=True)
            feat16 = F.interpolate(
                feat16, (h, w), mode='bilinear', align_corners=True)
            feat32 = F.interpolate(
                feat32, (h, w), mode='bilinear', align_corners=True)
